...
describe("photos API", () => {
  it("uploads, lists, and serves a photo", async () => {
    const token = await seed();
    const up = await app.request("/api/orders/po/photos", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: form("PHOTO-BYTES", "packed") }, env);
    ...
    const { photos } = await list.json<{ photos: { id: string; note: string | null }[] }>();
